...
function ViewFiles() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-gray-100 to-gray-200 p-6 flex flex-col items-center">
      <h1 className="text-4xl font-bold text-blue-800 mb-1">{subject}</h1>
      <h4 className="text-xl text-blue-400 mb-10">Year-{year}, {branch}</h4>

      {/* Option Selector */}
      <div className="w-full max-w-4xl mb-8">
        <h3 className="text-2xl font-semibold text-gray-700 mb-4 text-center">Options for {subject}</h3>
        <ul className="flex flex-wrap gap-4 justify-between w-full">
          {options.map((opt, index) => (
            <li key={index}>
              <button
                className={`w-52 py-3 rounded-lg shadow-md font-medium transition duration-300 transform 
                  ${option === opt ? "bg-blue-700 text-white" : "bg-gradient-to-r from-teal-500 to-blue-500 text-white hover:from-blue-500 hover:to-teal-500 hover:scale-105"}
                `}
                onClick={() => setOption(opt)}
              >
                {opt}
          ...
          {dropdownOpen && (
            <div className="absolute left-0 mt-2 bg-white border border-gray-300 rounded-lg shadow-lg w-48 z-10">
              {filters.map((filter, index) => (
                <button
                  key={index}
                  className={`block w-full text-left px-4 py-2 text-gray-700 hover:bg-gray-100 ${selectedFilter === filter ? "font-bold text-blue-600" : ""}`}
                  onClick={() => {
      ...
      {!loading && files.length === 0 && (
        <div className="flex flex-col items-center mt-10">
          <img src="https://img.icons8.com/ios/100/empty-box.png" alt="No files" className="w-24 h-24 opacity-50" />
          <p className="text-2xl font-semibold text-gray-600 mt-4">No files available currently...</p>
